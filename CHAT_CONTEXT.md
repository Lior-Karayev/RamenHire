...
- **GoatCounter + Vercel Web Analytics (added 2026-07-15) — permanent, consent-independent traffic baselines, run alongside GA4.** GA4 is structurally undercounted by design: it only loads after a visitor clicks "Accept" on the cookie consent banner (see below), and a live investigation on 2026-07-15 found GA4 had recorded **zero** users/sessions/events for 7 straight days (2026-07-09 through 07-15) — not because traffic stopped, but because essentially no visitor was clicking Accept. GoatCounter (`https://ramenhire.goatcounter.com`) and Vercel's built-in Web Analytics (visible in the Vercel dashboard, Hobby plan, free up to 50k events/month) both set no cookies and load unconditionally in `app/layout.tsx`, **outside** `CookieConsentBanner`'s gating entirely — they need no consent because they collect nothing that requires it. `lib/analytics.ts`'s `trackEvent()` helper now fires the same custom event (`apply_click`, `post_job_click`, `subscribe_submitted`, etc.) into both GA4 (if consented) and GoatCounter (always) from one call site. Having two independent, non-overlapping measurement methods also means one can sanity-check the other going forward — useful given GA4's silent blind spot went unnoticed for a week. GA4 stays as-is for the richer event/funnel data from visitors who do consent; these two are the always-on floor underneath it. Note: GoatCounter's own script has a built-in `localhost` skip (won't fire real hits from local dev — confirmed by design, not a bug), and Vercel Analytics no-ops with a console debug message in local dev builds — both only send real data from the deployed production domain.
